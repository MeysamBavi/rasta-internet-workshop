...
همهٔ subtabها باید نگاشت داشته باشند. ترتیب آن‌ها از خود Google Doc خوانده و در
`steps/.order.json` ثبت می‌شود.
...
لینک Google Docs به مسیری مانند `/games/router/` باید در پاراگرافی مستقل باشد.
این لینک در Markdown به iframe تبدیل می‌شود.
